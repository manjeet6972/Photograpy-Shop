...
const FlexContent = ({ifExists, endpoint: {title, heading, text, img, btn}}) => {
  return (
    <>
      <div className={`flex items-center justify-between lg:flex-col lg:justify-center
        ...
        <div className='flex items-center justify-center max-w-xl relative lg:max-w-none w-full'>
          <img src={img} alt={`img/heading`}
            className={`w-auto object-fill transitions-theme ${ifExists ? 'w-80 lg:h-56 md:h-70 sm:h-60 xsm:h-46 rotate-6 hover:-rotate-12' : 'h-72 lg:h-64 md:h-60 sm:h48 xsm:h-40 rotate-[19deg] hover:rotate-12'}`} />
        </div>
      </div>
    </>
  )
