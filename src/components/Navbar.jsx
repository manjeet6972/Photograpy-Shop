...
import logo from '../../dist/assets/logo-1-3173f479.png';
...
const Navbar = () => {
  ...
  return (
    <>
      <header className={!navState ? 'absolute top-7 left-0 right-0 opacity-100 z-50' : 'fixed top-0 left-0 right-0 h-[9vh] flex items-center opacity-100 z-[200] blur-effect-theme'}>
          ...
          <ul className='flex items-center justify-center gap-2'>
            ...
            <li className='grid items-center'>
              ...
              transition-all duration-300 relative'>
                ...
                <div className={`absolute top-4 right-0 shadow
                w-4 h-4 text-[0.65rem] leading-tight font-medium rounded-full flex
                items-center justify-center cursor-pointer hover:scale-110 transition-all duration-300 ${navState ? 'bg-slate-900 text-slate-100 shadow-slate-900' : 'bg-slate-100 text-slate-900 shadow-slate-100'}`}
                >{totalQTY}</div>
              </button>
            </li>
